...
function authenticateToken(
  ...
  if (token == null)
    return res
      .status(401)
      .json({ [MSG_TYPES.ERROR]: "No access token was received" });
  ...
  jwt.verify(token, env.ACCESS_TOKEN_SECRET, async (err: any, user: any) => {
    if (err) {
      ...
      if (err.name === "TokenExpiredError") {
        return res
          .status(401)
          .json({ [MSG_TYPES.ERROR]: "Access token has expired" });
      }
      return res
        .status(401)
        .json({ [MSG_TYPES.ERROR]: "Access token could not be verified" });
    }
    req.user = user;
    next();
  });
  ...
  if (body) {
    if (req.method !== "GET") {
      if (body.hasOwnProperty("role") && !isSuperuser(user)) {
        return res
          .status(401)
          .send({ [MSG_TYPES.ERROR]: ERRORS.NOT_SUPERUSER });
      }
      if (
        body.hasOwnProperty(
          "verified" ||
